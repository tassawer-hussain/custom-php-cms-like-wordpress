                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <?php 
                            $user_image = '';
                            $user_role = '';
                            $edit_user_query = "SELECT * FROM users WHERE username = '{$_SESSION['username']}'";
                            ...
                            while ($row = mysqli_fetch_assoc($edit_user)) {
                                ...
                                $user_role = array();
                                $query = "SELECT * FROM users_role WHERE u_id=$user_id";
                        ...
                        if(isset($_POST['update_profile'])){
                            ...
                            $update_query .= "WHERE user_id = '{$_SESSION['u_id']}'";
                            ...
                            if($is_updated_user) {
                                foreach($user_role as $r) {
                                    $delete_role = "DELETE FROM users_role WHERE u_id={$_SESSION['u_id']} AND r_id=$r";
                                    mysqli_query($connection, $delete_role);
                                }
                                foreach($u_role as $role) {
                                    $query = "INSERT INTO users_role(u_id, r_id) VALUES({$_SESSION['u_id']}, $role)";
                                    mysqli_query($connection, $query);
                                }
                                //die("If end before header");
